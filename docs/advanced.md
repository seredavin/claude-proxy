...
| `--mask-key-file` | `CLAUDE_PROXY_MASK_KEY_FILE` | — | Файл ключа маскирования: суррогаты IP и хостов одинаковы во всех процессах с этим ключом, см. [Ключ маскирования](#ключ-маскирования) |
...
| встроенный | IPv4-mapped IPv6: `::ffff:10.0.0.1`, `::ffff:0a00:0001` | `::ffff:` + суррогат вложенного IPv4 в той же записи, тот же, что у `10.0.0.1`; класс и `ip private` — по вложенному IPv4. NAT64 (`64:ff9b::a.b.c.d`) и `::a.b.c.d` маскируются как IPv6 целиком |
...
Таблица исчезает вместе с процессом. Без ключа маскирования суррогаты
выбираются случайно, и после перезапуска шлюза (или нового
`claude-proxy local` под `claude --resume`) те же адреса получают другие
суррогаты: модель видит ту же инфраструктуру под новыми именами, кэш
промптов теряется. Суррогаты прошлого процесса, если они попали в историю,
уйдут на апстрим и вернутся как обычный текст, без ошибок — но команда с
таким «адресом» выполнится буквально. С ключом маскирования суррогаты IP и
хостов после перезапуска те же — см. следующий раздел.

### Ключ маскирования

С ключом суррогаты IP-адресов и имён хостов не выбираются случайно, а
вычисляются из значения и ключа. Один адрес при одном ключе даёт один
суррогат везде: после перезапуска, в `claude --resume`, в параллельных
`claude-proxy local` и на любом звене с этим ключом.

```bash
claude-proxy gen-mask-key > ~/.config/claude-proxy/mask.key
chmod 600 ~/.config/claude-proxy/mask.key
# в ~/.config/claude-proxy/local.env:
CLAUDE_PROXY_MASK_KEY_FILE=/home/me/.config/claude-proxy/mask.key
```

Ключ действует только вместе с `--mask-rules`. Файл доступен группе или
остальным — отказ на старте с подсказкой `chmod 600`. В стартовом логе —
отпечаток ключа (`fingerprint=e232ab66`): по нему видно, что два процесса
работают с одним ключом. Сам ключ в лог не попадает.

| Значение | Суррогат с ключом | Обратим по ключу |
|---|---|---|
| приватный IPv4, ULA IPv6 | адрес того же диапазона; `/24` (`/64`) сохраняется; у IPv4 `.0` и `.255`, у IPv6 нулевой хост остаются на месте | да, без таблицы |
| имя хоста | `host-<шифртекст>.example`: base32 строчными, метки не длиннее 63 | да, без таблицы; имя в нижнем регистре |
| публичный IPv4 | адрес из `100.64.0.0/10` в одной `/24` с соседями | нет |
| публичный IPv6 | адрес из `2001:db8::/32` | нет |
| секреты, `regex` | как без ключа (`SECRETnnnn`, `ИМЯnnnn`) | нет |

Суррогат приватного адреса или хоста — это шифрование с сохранением
формата: владелец ключа восстанавливает исходное значение без живого
процесса (например, разбирая трассу), а без ключа суррогат не отличить от
случайного. Публичных сетей больше, чем помещается в пул суррогатов, поэтому
их сеть вычисляется хешем, а при совпадении двух реальных сетей вторая
получает следующую пробу — тогда её суррогат зависит от порядка появления
адресов. Совпадения начинаются примерно с сотни разных публичных `/24` в
одной метке.

Что стоит учитывать:

- Суррогаты одинаковы **для всех меток токена** с одним ключом: апстрим
  может сопоставить инфраструктуру разных команд на одном шлюзе. Изоляция
  таблиц при этом прежняя — клиент `analytics` не получит демаскированный
  суррогат `dev-team`. Разным командам — разные ключи.
- Обратная подстановка в ответе по-прежнему идёт только по таблице: адрес,
  который модель придумала сама, клиенту не «расшифровывается».
- Утечка ключа раскрывает приватные адреса и хосты во всех запросах, уже
  отправленных с ним. Держите ключ на машине с Claude Code (звено `local`).
- Ротация — новый ключ: все суррогаты меняются, единственное следствие —
  промах кэша промптов.
- Имя хоста длиннее ~225 символов в суррогат не помещается и получает
  случайный `host-N.example`, как без ключа.

На шлюзе под systemd: `install --mask-key-file …` записывает переменную в
`EnvironmentFile` и проверяет, что ключ прочитает сервисный пользователь;
если нет — отказ с подсказкой `chown claude-proxy <файл>` (не `setfacl`:
ACL открывает групповые биты режима, и ключ перестанет проходить проверку
прав).
...
- Тело запроса буферизуется целиком (в пределах `--max-body`), таблица
  ...
  загруженном шлюзе.
- Без ключа публичных адресов на метку — около 760 в документационных сетях, дальше
